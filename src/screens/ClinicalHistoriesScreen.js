...
import { TIPOS_CASO, ESTADOS_TRATAMIENTO } from '../utils/constants';
...
const ClinicalHistoriesScreen = ({ navigation }) => {
  ...
  const [form, setForm] = useState({
    numero: '', tipo: 'Consulta', clienteId: '', clienteNombre: '',
    clienteTelefono: '', estado: 'Pendiente', descripcion: '',
    fechaApertura: new Date(), movimientos: [], area: 'Civil',
  });
  ...
  useEffect(() => {
    const unsubAuth = onAuthChange((currentUser) => {
      if (currentUser) {
        ...
        const unsubCli = subscribeToCollection('pacientes', (data) => setPatients(data));
  ...
  const filtered = historias.filter((e) =>
    `${e.numero} ${e.pacienteNombre || e.clienteNombre} ${e.tipo} ${e.estado}`
  ...
  const selectPatient = (patient) => {
    setForm({
      ...
      clienteNombre: `${patient.nombre} ${patient.apellido}`,
      clienteTelefono: patient.telefono || '',
    });
  };
  ...
  const handleSave = async () => {
    ...
    if (editingHistoria) {
      await updateDocument('historias_clinicas', editingHistoria.id, data);
      if (form.clienteTelefono) {
        sendCaseUpdate(form.clienteTelefono, form.clienteNombre, form.numero, form.estado);
      }
    } else {
      await createDocument('historias_clinicas', data);
    }
    setModalVisible(false);
    resetForm();
  };

  const generatePDF = async (hist) => {
    const result = await generateCaseReportPDF(hist);
    if (result.success) {
      await sharePDF(result.uri, `Historia_${hist.numero}`);
    } else {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => {
      setEditingHistoria(item);
      setForm({
        numero: item.numero, tipo: item.tipo || 'Consulta',
        clienteId: item.clienteId || item.pacienteId || '',
        clienteNombre: item.clienteNombre || item.pacienteNombre || '',
        clienteTelefono: item.clienteTelefono || item.pacienteTelefono || '',
        estado: item.estado || 'Pendiente', descripcion: item.descripcion || '',
        fechaApertura: item.fechaApertura ? new Date(item.fechaApertura) : new Date(),
        movimientos: item.movimientos || [], area: item.area || 'Civil',
      });
      setModalVisible(true);
    }}>
      <Card icon="📋" title={`#${item.numero}`} subtitle={item.clienteNombre || item.pacienteNombre} badge={item.estado}>
  ...
  return (
    <View style={styles.container}>
      <Header title="Expedientes" subtitle={`${historias.length} registrados`}
      ...
      <FlatList data={filtered} renderItem={renderItem} keyExtractor={(item) => item.id}
        ...
        ListEmptyComponent={<Card><Text style={styles.emptyText}>No hay expedientes registrados</Text></Card>} />

      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <ScrollView style={styles.modalContent}>
            <Text style={styles.modalTitle}>{editingHistoria ? 'Editar Expediente' : 'Nuevo Expediente'}</Text>
            {!editingHistoria && (
              <View>
                <Text style={styles.fieldLabel}>Cliente:</Text>
                <FlatList data={patients} horizontal showsHorizontalScrollIndicator={false}
                  renderItem={({ item }) => (
                    <TouchableOpacity
            ...
            <Text style={styles.fieldLabel}>Tipo de Caso:</Text>
