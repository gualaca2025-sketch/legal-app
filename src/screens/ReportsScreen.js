...
const ReportsScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const unsubAuth = onAuthChange((currentUser) => {
      if (currentUser) {
        setUser(currentUser);
        const unsub1 = subscribeToCollection('historias_clinicas', (data) => setHistorias(data),
  ...
  const tiposCount = {};
  historias.forEach((e) => { tiposCount[e.tipo] = (tiposCount[e.tipo] || 0) + 1; });

  const estadosCount = {};
  historias.forEach((e) => { estadosCount[e.estado] = (estadosCount[e.estado] || 0) + 1; });
  ...
  const tratamientosPorTipo = Object.entries(tiposCount).slice(0, 7);
  const pieData = tratamientosPorTipo.map(([name, count], i) => ({
    name: name.length > 10 ? name.substring(0, 10) + '...' : name,
    count,
    color: ['#00897B', '#26C6DA', '#388E3C', '#D32F2F', '#F57C00', '#1976D2', '#7B1FA2'][i % 7],
    legendFontColor: COLORS.text,
    legendFontSize: 11,
  }));

  const generarReportePDF = async () => {
    const historiasHTML = historias.map((e) =>
      `<tr><td>${e.numero}</td><td>${e.pacienteNombre}</td><td>${e.tipo}</td><td><span class="status-badge status-${(e.estado || '').toLowerCase()}">${e.estado}</span></td></tr>`
    ).join('');

    const cobrosHTML = cobros.map((c) =>
      `<tr><td>${c.numeroFactura || 'N/A'}</td><td>${c.pacienteNombre}</td><td>B/. ${c.monto}</td><td><span class="status-badge status-${(c.estado || '').toLowerCase()}">${c.estado}</span></td></tr>`
    ).join('');

    const content = `
<h3 style="color:#00897B;">Resumen General del Bufete</h3>
<div class="info-grid">
  <div class="info-item"><label>Total Historias</label><div class="value">${historias.length}</div></div>
  ...
  <div class="info-item"><label>Activos</label><div class="value">${historias.filter((e) => e.estado === 'En Progreso').length}</div></div>
</div>

<h3 style="color:#00897B;margin-top:20px;">Expedientes</h3>
<table><tr><th>Número</th><th>Cliente</th><th>Tipo</th><th>Estado</th></tr>${historiasHTML}</table>

<h3 style="color:#00897B;margin-top:20px;">Cobros</h3>
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card title="Casos por Tipo de Derecho">
          {pieData.length > 0 ? (
            <PieChart
              data={pieData}
        ...
        <View style={styles.statsGrid}>
          <Card style={styles.statItem}>
            ...
            <Text style={styles.statNumber}>{historias.length}</Text>
            <Text style={styles.statLabel}>Total Expedientes</Text>
          </Card>
          <Card style={styles.statItem}>
            <Text style={styles.statIcon}>⚖️</Text>
            <Text style={styles.statNumber}>{historias.filter((e) => e.estado === 'En Progreso').length}</Text>
