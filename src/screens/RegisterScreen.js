...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    if (form.password.length > 12) {
      Alert.alert('Error', 'La contraseña no debe tener más de 12 caracteres');
      return;
    }
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>Únase a nuestro bufete</Text>
        </View>
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.inputContainer}>
            <TextInput
              style={styles.input}
              placeholder="Contraseña * (6-12 caracteres)"
              placeholderTextColor={COLORS.disabled}
              value={form.password}
          ...
          <View style={styles.rolContainer}>
            {['abogado', 'asistente', 'recepcionista', 'admin'].map((rol) => (
              <TouchableOpacity
                key={rol}
                style={[
                  styles.rolButton,
