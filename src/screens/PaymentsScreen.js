...
const PaymentsScreen = ({ navigation }) => {
  ...
  const [form, setForm] = useState({
    pacienteId: '', pacienteNombre: '', pacienteTelefono: '',
    tipo: 'consulta', monto: '', descripcion: '', metodoPago: 'Transferencia Bancaria',
  ...
  const handleSave = async () => {
    if (!form.pacienteNombre || !form.monto) {
      Alert.alert('Error', 'Cliente y monto son obligatorios');
      return;
    }
    const data = { ...form, monto: parseFloat(form.monto), doctorId: user.uid };
    const result = await createDocument('cobros', data);
    if (result.success) {
      if (form.pacienteTelefono) {
        sendInvoiceNotification(form.pacienteTelefono, form.pacienteNombre, form.numeroFactura, form.monto);
      }
  ...
  const markAsPaid = async (payment) => {
    ...
    if (payment.pacienteTelefono) {
      sendWhatsAppMessage(payment.pacienteTelefono,
          ...
          <View style={styles.payActions}>
            ...
            {item.pacienteTelefono && (
              <TouchableOpacity style={styles.remindBtn} onPress={() =>
                sendPaymentReminder(item.pacienteTelefono, item.pacienteNombre, item.monto, item.fechaVencimiento)}>
          ...
          <ScrollView style={styles.modalContent}>
            ...
            <View>
              ...
              <FlatList data={patients} horizontal showsHorizontalScrollIndicator={false}
                renderItem={({ item }) => (
                  <TouchableOpacity style={[styles.patientChip, form.pacienteId === item.id && styles.patientChipActive]}
