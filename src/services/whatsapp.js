...
export const sendAppointmentConfirmation = async (phone, cliente, fecha, hora) => {
  const message = `Hola ${cliente}, ✅ *Cita Confirmada*\n\n` +
    `Estimado(a) ${cliente}, su cita con *José María Araúz Barraza* ha sido confirmada.\n\n` +
    `📅 *Fecha:* ${fecha}\n⏰ *Hora:* ${hora}\n📍 *Lugar:* David, Chiriquí\n\n` +
    `Si necesita reagendar, contáctenos al 6615-6227.\n\n` +
    `*José María Araúz Barraza* - Su confianza, nuestro compromiso.`;
...
export const sendWelcomeMessage = async (phone, nombre) => {
  const message = `🎉 *Bienvenido a José María Araúz Barraza*\n\n` +
...
export const getChatbotResponse = async (message) => {
  ...
  if (lowerMsg.includes('cita') || lowerMsg.includes('agendar')) {
    return 'Para agendar una cita, por favor proporcione:\n' +
      ...
      'O puede llamarnos directamente al 6615-6227.';
  }
  ...
  return '🤖 Gracias por contactar a *José María Araúz Barraza*.\n\n' +
    'Puedo ayudarle con:\n' +
    '• 📅 Agendar citas\n' +
